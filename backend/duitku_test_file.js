...
async function test(methodName, methodValue) {
  ...
  try {
    ...
    logOutput += `[${methodName}] SUCCESS: ${response.data.statusMessage}\n`;
  } catch (err) {
    let msg = err.response?.data?.Message || err.response?.data?.statusMessage || err.message || "";
    if (typeof msg !== 'string') msg = JSON.stringify(msg);
    msg = msg.replace(/\s+/g, ' ').trim();
    logOutput += `[${methodName}] FAILED: ${msg}\n`;
  }
}
...
async function run() {
  ...
  fs.writeFileSync('duitku_results.txt', logOutput, 'utf8');
