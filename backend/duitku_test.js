...
async function test(methodName, methodValue) {
  ...
  try {
    ...
    console.log(`[${methodName}] Result:`, response.data.statusCode, response.data.statusMessage, response.data.paymentUrl || '');
  } catch (err) {
    console.log(`[${methodName}] Error:`, err.response?.data?.Message || err.response?.data?.statusMessage || err.message);
  }
}
