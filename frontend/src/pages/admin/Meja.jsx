...
function Meja() {
    ...
    const downloadQR = async () => {
        ...
        try {
            ...
            // Setup Promise untuk Image Onload
            const imgLoadPromise = new Promise((resolve, reject) => {
                img.onload = () => {
                    ...
                    // Ambil base64 tanpa header data:image/png;base64,
            ...
            // Tunggu selesai gambar dilukis dan jadi base64 string
            const resultBase64 = await imgLoadPromise;
            const filename = showGeneralQR ? 'qr-menu-umum.png' : `qr - meja - ${selectedQRTable?.number}.png`;

            // Download file secara native Capacitor atau Web
            await downloadFile({
                filename: filename,
            ...
            console.error('Error saat unduh QR:', error);
            toast.error('Gagal mengunduh QR', { id: toastId });
        } finally {
                    ...
                    <div className="modal-overlay">
                        <div className="glass rounded-2xl p-6 w-full max-w-sm animate-scale-up">
                            ...
                            <div ref={qrRef} className="bg-white rounded-xl p-6 flex flex-col items-center">
                                <QRCodeSVG
                                    value={showGeneralQR
                                        ? `${getBaseUrl()}${tenantSlug ? `/${tenantSlug}` : ''}`
                                        : `${getBaseUrl()}${tenantSlug ? `/${tenantSlug}` : ''}?meja=${selectedQRTable?.number}`
                                    }
                                    size={200}
                                    level="H"
                                    includeMargin={true}
                            ...
                            <div className="grid grid-cols-2 gap-3 mt-4">
                                <button
                                    ...
                                    className="py-3 rounded-xl bg-green-500/20 text-green-400 font-medium hover:bg-green-500/30 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                                >
